...
def main(argv):
    ...
    model = DDPG.load("../pybullet_logs/kukapush_ddpg/"+ policy_name)

    kukaenv = kukaPushGymEnv(urdfRoot=robot_data.getDataPath(), renders=True, useIK=0, numControlledJoints = numControlledJoints, fixedPositionObj = fixed, includeVelObs = True)
    for i in range(10):
        obs = kukaenv.reset()
        dones = False
        while not dones:
